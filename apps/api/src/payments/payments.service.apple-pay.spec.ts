...
describe('PaymentsService Apple Pay flows', () => {
  ...
  it('exchanges Apple Pay token and stores a default Apple Pay payment method', async () => {
    applePayClientMock.exchangeToken.mockResolvedValue({
      network: 'visa',
      paymentMethodToken: 'apple-tokenized-card-1',
    });
    ...
    await expect(
      service.exchangeApplePayToken(USER_ID, {
        cardLast4: '4242',
        isDefault: true,
    ...
    expect(applePayClientMock.exchangeToken).toHaveBeenCalledWith({
      paymentToken: 'apple-pay-payment-token',
      transactionIdentifier: 'apple-pay-transaction-1',
    });
  ...
  it('uses ArCa pre-authorization flow for one Apple Pay session payment', async () => {
  ...
  it('uses ArCa capture flow for one authorized Apple Pay payment', async () => {
  ...
  it('uses ArCa refund flow for one failed Apple Pay session payment', async () => {
    prismaMock.payment.findUnique.mockResolvedValue(
      buildPaymentRecord({
        capturedAmount: 3800,
